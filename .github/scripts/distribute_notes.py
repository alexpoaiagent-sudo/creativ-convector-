...
def process_note(note_path):
    ...
    # Сохранить копию оригинала в папку "Обработанные"
    PROCESSED_PATH.mkdir(parents=True, exist_ok=True)
    processed_date_folder = PROCESSED_PATH / datetime.now().strftime("%Y-%m-%d")
    processed_date_folder.mkdir(exist_ok=True)
    processed_path = processed_date_folder / filename
    with open(processed_path, 'w', encoding='utf-8') as f:
        f.write(content)  # Сохраняем оригинал без frontmatter
    print(f"📋 Копия сохранена в: {processed_path.relative_to(VAULT_PATH)}")
...
def create_report(processed_notes):
    ...
    # Группировка по проектам для графика
    by_project_temp = {}
    for note in processed_notes:
        project = note['project']
        if project not in by_project_temp:
            by_project_temp[project] = 0
        by_project_temp[project] += 1

    # Создать Mermaid pie chart
    mermaid_chart = "```mermaid\npie title Распределение по проектам\n"
    for project, count in by_project_temp.items():
        mermaid_chart += f'    "{project}" : {count}\n'
    mermaid_chart += "```\n"
...
|--------|-------------------|
"""
    for project, count in sorted(by_project_temp.items(), key=lambda x: x[1], reverse=True):
...
| Обработано заметок | {len(processed_notes)} |
| Дата обработки | {now.strftime('%Y-%m-%d %H:%M')} |
